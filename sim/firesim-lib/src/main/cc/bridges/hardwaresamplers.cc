...
base_profiler::base_profiler(std::vector<std::string> &args, struct traceInfo info, std::string name, bool customResult)
  ...
  if (info.coreWidth <= 0 || info.coreWidth > 4) {
    throw std::invalid_argument(std::string("Invalid core configuration detected. Please set core width and rob depth via core parameters. (Supported core width <= 4)"));
  }
